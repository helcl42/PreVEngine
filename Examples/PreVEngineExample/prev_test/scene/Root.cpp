...
Root::Root(prev::core::device::Device& device, bool colorManaged, uint32_t viewCount)
    : SceneNode()
    , m_device{ device }
...
void Root::Init()
{
    ...
    // auto skyBox = std::make_shared<sky::SkyBox>(m_device, m_colorManaged);
    // AddChild(skyBox);

    auto sky = std::make_shared<sky::Sky>(m_device, m_colorManaged);
    AddChild(sky);
    ...
    //             auto robot = std::make_shared<robot::CubeRobot>(m_device, m_colorManaged, glm::vec3(i * DISTANCE, j * DISTANCE, k * DISTANCE), glm::quat(1, 0, 0, 0), glm::vec3(1, 1, 1), prev_test::common::AssetManager::Instance().GetAssetPath("Textures/texture.jpg"));
    //             AddChild(robot);
    //         }
    //     }
    // }
...
#else
    auto player = std::make_shared<Player>(m_device, m_colorManaged, glm::vec3(90.0f, 0.0f, 100.0f), glm::quat(glm::radians(glm::vec3(0.0f, 90.0f, 0.0f))), glm::vec3(0.06f));
#endif
    AddChild(player);

    const uint32_t TERRAIN_GRID_MAX_X{ 3 };
    const uint32_t TERRAIN_GRID_MAX_Z{ 3 };
    auto terrainManager = std::make_shared<terrain::TerrainManager>(m_device, m_colorManaged, TERRAIN_GRID_MAX_X, TERRAIN_GRID_MAX_Z);
    AddChild(terrainManager);

    auto water = std::make_shared<water::WaterManager>(m_device, m_colorManaged, TERRAIN_GRID_MAX_X, TERRAIN_GRID_MAX_Z);
    AddChild(water);

    auto sun = std::make_shared<sky::Sun>(m_device, m_colorManaged);
    AddChild(sun);
