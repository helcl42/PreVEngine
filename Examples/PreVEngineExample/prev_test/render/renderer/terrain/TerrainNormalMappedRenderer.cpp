...
void TerrainNormalMappedRenderer::Init()
{
    ...
    m_shader = prev::render::shader::ShaderBuilder{ m_device }
        .AddShaderStagePaths({
            { GFX_SHADER_STAGE_VERTEX, prev_test::common::ShaderAssetManager::Instance().GetAssetPath(m_device.GetAdapter().GetInfo().backend, "terrain/terrain_normal_mapped_vert") },
            { GFX_SHADER_STAGE_FRAGMENT, prev_test::common::ShaderAssetManager::Instance().GetAssetPath(m_device.GetAdapter().GetInfo().backend, "terrain/terrain_normal_mapped_frag") }
        })
...
void TerrainNormalMappedRenderer::BeginFrame(const NormalRenderContext& renderContext)
{
...
void TerrainNormalMappedRenderer::EndFrame(const NormalRenderContext& renderContext)
{
    m_shader->EndFrame();
