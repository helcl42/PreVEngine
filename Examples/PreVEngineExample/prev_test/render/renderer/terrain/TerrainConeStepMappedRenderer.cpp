...
void TerrainConeStepMappedRenderer::Init()
{
    ...
    m_shader = prev::render::shader::ShaderBuilder{ m_device }
        .AddShaderStagePaths({
            { GFX_SHADER_STAGE_VERTEX, prev_test::common::ShaderAssetManager::Instance().GetAssetPath(m_device.GetAdapter().GetInfo().backend, "terrain/terrain_cone_step_mapped_vert") },
            { GFX_SHADER_STAGE_FRAGMENT, prev_test::common::ShaderAssetManager::Instance().GetAssetPath(m_device.GetAdapter().GetInfo().backend, "terrain/terrain_cone_step_mapped_frag") }
        })
        ...
        .AddBindGroupEntries({
            ...
            prev::render::shader::ShaderBuilder::BindGroupEntry::Texture("heightTextures", 6, GFX_SHADER_STAGE_FRAGMENT, GFX_TEXTURE_VIEW_TYPE_2D_ARRAY),
...
void TerrainConeStepMappedRenderer::BeginFrame(const NormalRenderContext& renderContext)
{
...
void TerrainConeStepMappedRenderer::EndFrame(const NormalRenderContext& renderContext)
{
    m_shader->EndFrame();
