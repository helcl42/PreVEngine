...
ConeStepMappedRenderer::ConeStepMappedRenderer(prev::core::device::Device& device, prev::render::pass::RenderPass& renderPass, prev::scene::IScene& scene)
    : m_device{ device }
...
void ConeStepMappedRenderer::Init()
{
    ...
    m_shader = prev::render::shader::ShaderBuilder{ m_device }
        .AddShaderStagePaths({
            { GFX_SHADER_STAGE_VERTEX, prev_test::common::AssetManager::Instance().GetAssetPath("Shaders/normal/cone_step_mapped_vert.spv") },
            { GFX_SHADER_STAGE_FRAGMENT, prev_test::common::AssetManager::Instance().GetAssetPath("Shaders/normal/cone_step_mapped_frag.spv") }
        })
...
{
    if (!node->GetTags().HasAll({ TAG_RENDER_CONE_STEP_MAPPED_COMPONENT })) {
        return;
    }
