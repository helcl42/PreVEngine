...
void main()
{
	...
	const vec3 normal = uboFS.hasNormalMap != 0 ? NormalMapping(normalTexture, normalSampler, uv, ddx, ddy) : inNormal;
	const vec4 textureColor = textureGrad(sampler2D(colorTexture, colorSampler), uv, ddx, ddy);
