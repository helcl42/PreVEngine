...
struct AnimationTexturelessFSParams_std140_0
{
...
@binding(1) @group(0) var<uniform> uboFS_0 : AnimationTexturelessFSParams_std140_0;
...
fn fragmentMain( _S12 : pixelInput_0, @builtin(position) position_1 : vec4<f32>) -> pixelOutput_0
{
    ...
    var resultColor_0 : vec4<f32> = mix(vec4<f32>(uboFS_0.fogColor_0.xyz, 1.0f), vec4<f32>(totalDiffuse_0 * vec3<f32>(shadow_2) + vec3<f32>(uboFS_0.lightning_0.ambientFactor_0), 1.0f) * uboFS_0.material_0.color_1 + vec4<f32>(totalSpecular_0 * vec3<f32>(shadow_2), 0.0f), vec4<f32>(_S12.visibility_0));
