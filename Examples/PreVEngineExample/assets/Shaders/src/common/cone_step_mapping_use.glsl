...
vec2 ConeStepMapping(in texture2D coneMapTexture, in sampler coneMapSampler, in float heightScale, in uint numLayers, in vec2 uv, in vec2 ddx, in vec2 ddy, in vec3 texDir3D)
{
    vec2 R = normalize(vec2(length(texDir3D.xy), texDir3D.z));
    vec2 P = R * heightScale / texDir3D.z;

    vec2 textureSize = textureSize(sampler2D(coneMapTexture, coneMapSampler), 0);
    ...
    for (uint i = 0; i < numLayers; ++i)
    {
        ...
        vec2 heightAndCone = clamp(textureGrad(sampler2D(coneMapTexture, coneMapSampler), samplePoint.xy, ddx, ddy).rg, 0.0, 1.0);
...
vec2 RelaxedConeStepMapping(in texture2D coneMapTexture, in sampler coneMapSampler, in float heightScale, in uint numLayers, in vec2 uv, in vec2 ddx, in vec2 ddy, in vec3 texDir3D)
{
    ...
    for(uint i = 0; i < numLayers; ++i)
    {
        vec2 heightAndCone = clamp(textureGrad(sampler2D(coneMapTexture, coneMapSampler), pos.xy, ddx, ddy).rg, 0.0, 1.0);
