...
struct RenderableText3d {
    static constexpr inline uint32_t ModelCount{ 2 };

    std::shared_ptr<prev_test::render::font::WorldSpaceText> text{};
...
class IFont3dRenderComponent {
public:
    virtual std::shared_ptr<prev_test::render::font::FontMetadata> GetFontMetadata() const = 0;

    virtual const std::map<uint32_t, RenderableText3d>& GetRenderableTexts() const = 0;

    virtual void AddText(const uint32_t key, const std::shared_ptr<prev_test::render::font::WorldSpaceText>& text) = 0;
...
public:
    virtual ~IFont3dRenderComponent() = default;
};
} // namespace prev_test::component::font
