...
NormalMappedRenderer::NormalMappedRenderer(prev::core::device::Device& device, prev::core::memory::Allocator& allocator, prev::render::pass::RenderPass& renderPass, prev::scene::IScene& scene)
    : m_device{ device }
    ...
    std::function<void(const prev_test::render::MeshNode&)> RenderMeshNode = [&](const prev_test::render::MeshNode& meshNode) {
        ...
        for (const auto meshPartIndex : meshNode.meshPartIndices) {
            ...
            if (material->HasImageBuffer(NORMAL_INDEX)) {
                m_shader->Bind("normalSampler", *material->GetImageBuffer(NORMAL_INDEX), *material->GetSampler(COLOR_INDEX), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
            }
