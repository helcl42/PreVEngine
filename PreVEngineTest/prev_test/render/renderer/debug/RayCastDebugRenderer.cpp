...
void RayCastDebugRenderer::Init()
{
    ...
    m_uniformsPoolGS = std::make_unique<prev::render::buffer::UniformBufferRing<UniformsGS>>(m_allocator);
    m_uniformsPoolGS->AdjustCapactity(m_descriptorCount, static_cast<uint32_t>(m_device.GetGPU().GetProperties().limits.minUniformBufferOffsetAlignment));
