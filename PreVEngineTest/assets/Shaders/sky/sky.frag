...
vec4 RaymarchToCloud(in vec3 startPos, in vec3 endPos, in vec3 bg, out vec4 cloudPos)
{
...
void main()
{
	...
	backgroundColor.rgb = backgroundColor.rgb * (1.0 - cloudColor.a) + cloudColor.rgb;
	backgroundColor.a = 1.0;

	vec4 bloomColor = vec4(GetSunColor(worldDir, 128) * 1.3, 1.0);

	float cloudAlphaness = clamp(cloudColor.a, 0.2, 1.0);
