...
layout(location = 4) out vec3 outToLightVectors[MAX_LIGHT_COUNT];
layout(location = 8) out vec3 outToCameraVector;
layout(location = 9) out float outVisibility;
...
void main() 
{
	...
	for (uint i = 0; i < uboVS.realCountOfLights; i++)
	{
		outToLightVectors[i] = uboVS.lightPositions[i].xyz - worldPosition.xyz;
	}
