...
void SelectionDebugRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::debug::shader::SelectionDebugShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/selection_debug_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/selection_debug_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Selection Debug Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::debug::pipeline::SelectionDebugPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
