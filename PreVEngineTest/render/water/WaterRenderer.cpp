...
namespace prev_test::render::water {
...
void WaterRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::water::shader::WaterShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/water_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/water_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Water Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::water::pipeline::WaterPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
