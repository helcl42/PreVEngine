...
void NormalMappedRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::normal::shader::NormalMappedShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/normal_mapped_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/normal_mapped_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Normal Mapped Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::normal::pipeline::NormalMappedPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
