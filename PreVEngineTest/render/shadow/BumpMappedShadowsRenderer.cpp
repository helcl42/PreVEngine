...
void BumpMappedShadowsRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::shadow::shader::BumpMappedShadowsShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/bump_mapped_shadows_vert.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Bump Mapped Shadows Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::shadow::pipeline::BumpMappedShadowsPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
