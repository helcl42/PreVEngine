...
void AnimationBumpMappedShadowsRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::shadow::shader::AnimatedBumplMappedShadowsShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/animation_bump_mapped_shadows_vert.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Animation Bump Mapped Shadows Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::shadow::pipeline::AnimatedBumpMappedShadowsPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
