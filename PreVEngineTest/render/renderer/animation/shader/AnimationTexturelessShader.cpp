#include "AnimationTexturelessShader.h"
...
AnimationTexturelessShader::AnimationTexturelessShader(const VkDevice device)
    : Shader(device)
{
}

std::map<VkShaderStageFlagBits, std::string> AnimationTexturelessShader::GetPaths()
{
    return {
        { VK_SHADER_STAGE_VERTEX_BIT, prev_test::common::AssetManager::Instance().GetAssetPath("Shaders/animation/animation_textureless_vert.spv") },
        { VK_SHADER_STAGE_FRAGMENT_BIT, prev_test::common::AssetManager::Instance().GetAssetPath("Shaders/animation/animation_textureless_frag.spv") }
    };
}

std::vector<VkVertexInputBindingDescription> AnimationTexturelessShader::CreateVertexInputBindingDescriptors() const
{
    return {
...
std::vector<VkVertexInputAttributeDescription> AnimationTexturelessShader::CreateInputAttributeDescriptors() const
{
    return {
...
std::vector<prev::render::shader::Shader::DescriptorSet> AnimationTexturelessShader::CreateDescriptorSets() const
{
    return {
        // vertex shader
...
std::vector<prev::render::shader::Shader::PushConstantBlock> AnimationTexturelessShader::CreatePushConstantBlocks() const
{
    return {};
}
} // namespace prev_test::render::renderer::animation::shader
