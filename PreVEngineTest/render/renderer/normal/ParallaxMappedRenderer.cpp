...
void ParallaxMappedRenderer::Init()
{
    ...
    m_pipeline = std::make_unique<pipeline::ParallaxMappedPipeline>(*device, *m_shader, *m_renderPass);
    m_pipeline->Init();
...
void ParallaxMappedRenderer::Render(const prev::render::RenderContext& renderContext, const std::shared_ptr<prev::scene::graph::ISceneNode>& node, const NormalRenderContextUserData& renderContextUserData)
{
    if (node->GetTags().HasAll({ TAG_RENDER_PARALLAX_MAPPED_COMPONENT, TAG_TRANSFORM_COMPONENT })) {
        bool visible{ true };
...
void ParallaxMappedRenderer::RenderMeshNode(const prev::render::RenderContext& renderContext, const std::shared_ptr<prev::scene::graph::ISceneNode>& node, const NormalRenderContextUserData& renderContextUserData, const prev_test::render::MeshNode& meshNode)
{
