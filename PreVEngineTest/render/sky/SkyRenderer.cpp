...
void SkyRenderer::Init()
{
    ...
    // compute sky
    m_conmputeSkyShader = shaderFactory.CreateShaderFromFiles<prev_test::render::sky::shader::SkyShader>(*device, { { VK_SHADER_STAGE_COMPUTE_BIT, AssetManager::Instance().GetAssetPath("Shaders/sky_comp.spv") } });
    m_conmputeSkyShader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Sky Compute Shader created\n");

    m_computeSkyPipeline = std::make_unique<prev_test::render::sky::pipeline::SkyPipeline>(*device, *m_conmputeSkyShader);
    ...
    m_conmputeSkyPostProcessShader = shaderFactory.CreateShaderFromFiles<prev_test::render::sky::shader::SkyPostProcessShader>(*device, { { VK_SHADER_STAGE_COMPUTE_BIT, AssetManager::Instance().GetAssetPath("Shaders/sky_post_process_comp.spv") } });
    m_conmputeSkyPostProcessShader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Sky PostProcess Compute Shader created\n");

    m_computeSkyPostProcessPipeline = std::make_unique<prev_test::render::sky::pipeline::SkyPostProcessPipeline>(*device, *m_conmputeSkyPostProcessShader);
    ...
    // compositor
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::common::shader::ScreenSpaceShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/screen_space_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/screen_space_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Sky Screen Space Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::common::pipeline::ScreenSpacePipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
