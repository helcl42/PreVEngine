...
void SunRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::sky::shader::SunOcclusionShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/sun_occlusion_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/sun_occlusion_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Sun Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::sky::pipeline::SunOcclusionPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
...
void SunRenderer::AfterRender(const prev::render::RenderContext& renderContext, const NormalRenderContextUserData& renderContextUserData)
{
    ...
    prev::event::EventChannel::Broadcast(prev_test::render::sky::SunVisibilityEvent{ ratio });
}
