...
void LensFlareRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::sky::shader::FlareShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/lens_flare_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/lens_flare_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("LensFlare Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::sky::pipeline::FlarePipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
...
void LensFlareRenderer::operator()(const prev_test::render::sky::SunVisibilityEvent& evt)
{
