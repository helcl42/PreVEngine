...
namespace prev_test::render::particle {
...
void ParticlesRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::particle::shader::ParticlesShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/particles_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/particles_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Particles Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::particle::pipeline::ParticlesPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
