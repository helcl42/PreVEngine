...
void AnimationConeStepMappedRenderer::Init()
{
    ...
    m_shader = shaderFactory.CreateShaderFromFiles<prev_test::render::animation::shader::AnimationConeStepMappedShader>(*device, { { VK_SHADER_STAGE_VERTEX_BIT, AssetManager::Instance().GetAssetPath("Shaders/animation_cone_step_mapped_vert.spv") }, { VK_SHADER_STAGE_FRAGMENT_BIT, AssetManager::Instance().GetAssetPath("Shaders/animation_cone_step_mapped_frag.spv") } });
    m_shader->AdjustDescriptorPoolCapacity(m_descriptorCount);

    LOGI("Animation Cone Step Mapped Shader created\n");

    m_pipeline = std::make_unique<prev_test::render::animation::pipeline::AnimationConeStepMappedPipeline>(*device, *m_renderPass, *m_shader);
    m_pipeline->Init();
