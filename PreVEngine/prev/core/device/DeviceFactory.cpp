...
std::unique_ptr<Device> DeviceFactory::Create(const Adapter& adapter, const std::vector<std::string>& extensions, const std::vector<std::string>& nativeExtensions) const
{
    ...
    GfxDevice gfxDevice{};
    if (gfxAdapterCreateDevice(adapter, &deviceDesc, &gfxDevice) != GFX_RESULT_SUCCESS) {
    ...
    const auto queueFamilies{ adapter.GetQueueFamilies() };
    const int32_t familyIndex{ adapter.FindQueueFamily(GFX_QUEUE_FLAG_GRAPHICS) };
    ...
    // Present shares the graphics family (Adapters::Find guarantees surface support on it).
    ...
    return std::make_unique<Device>(adapter, gfxDevice, std::move(queues), extensions);
}
} // namespace prev::core::device
